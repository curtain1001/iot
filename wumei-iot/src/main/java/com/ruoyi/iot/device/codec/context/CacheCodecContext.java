package com.ruoyi.iot.device.codec.context;
...
import com.ruoyi.iot.device.message.DeviceMessageReply;
import com.ruoyi.iot.device.message.RepayableDeviceMessage;
...
class CacheCodecContext implements CodecContext {
	...
	@Override
	public void cacheDownstream(Object key, RepayableDeviceMessage<? extends DeviceMessageReply> message,
			Duration ttl) {
	...
	@Override
	public <T extends RepayableDeviceMessage<? extends DeviceMessageReply>> Optional<T> getDownstream(Object key,
			boolean remove) {
		return Optional.ofNullable(remove ? caches.remove(key) : caches.get(key)).map(cache -> {
			if (cache.isAlive()) {
