...
import net.pingfang.iot.core.domain.News;
import net.pingfang.iot.core.mapper.NewsMapper;
import net.pingfang.iot.core.model.CategoryNews;
import net.pingfang.iot.core.service.INewsService;
	...
	public List<CategoryNews> selectTopNewsList() {
		...
		for (int i = 0; i < newsList.size(); i++) {
			...
			for (int j = 0; j < categoryNewsList.size(); j++) {
				if (newsList.get(i).getCategoryId().longValue() == categoryNewsList.get(j).getCategoryId()
						.longValue()) {
