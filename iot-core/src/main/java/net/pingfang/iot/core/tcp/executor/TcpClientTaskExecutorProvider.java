...
import net.pingfang.iot.core.network.DefaultNetworkType;
import net.pingfang.iot.core.network.NetworkManager;
...
public class TcpClientTaskExecutorProvider implements TaskExecutorProvider {
	...
	class TcpTaskExecutor extends AbstractTaskExecutor {
		...
		public void validate() {
			FastBeanCopier.copy(context.getJob().getConfiguration(), new TcpClientTaskConfiguration()).validate();
		}
		...
		protected Disposable doStart() {
			...
			if (config.getType() == PubSubType.producer) {
				disposable.add(context.getInput().accept()
						.flatMap(data -> clientManager
								.<TcpClient>getNetwork(DefaultNetworkType.TCP_CLIENT, config.getClientId())
								.flatMapMany(client -> RuleDataCodecs.getCodec(TcpMessage.class)
										.map(codec -> codec.decode(data, config.getPayloadType()).cast(TcpMessage.class)
												.switchIfEmpty(Mono.fromRunnable(() -> context.getLogger()
														.warn("can not decode rule data to tcp message:{}", data))))
										.orElseGet(() -> Flux
												.just(new TcpMessage(config.getPayloadType().write(data.getData()))))
										.flatMap(client::send).onErrorContinue((err, r) -> {
			...
			if (config.getType() == PubSubType.consumer) {
				disposable.add(clientManager.<TcpClient>getNetwork(DefaultNetworkType.TCP_CLIENT, config.getClientId())
						.switchIfEmpty(Mono.fromRunnable(
								() -> context.getLogger().error("tcp client {} not found", config.getClientId())))
						.flatMapMany(TcpClient::subscribe)
						.doOnNext(msg -> context.getLogger().info("received tcp client message:{}",
								config.getPayloadType().read(msg.getPayload())))
						.map(r -> RuleDataCodecs.getCodec(TcpMessage.class)
								.map(codec -> codec.encode(r, config.getPayloadType())).orElse(r.getPayload()))
