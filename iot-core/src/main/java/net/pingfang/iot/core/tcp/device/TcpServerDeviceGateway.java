package net.pingfang.iot.core.tcp.device;//package org.example.network.tcp.device;
//
...
//
//	public TcpServerDeviceGateway(String id, String protocol, ProtocolSupports supports, DeviceRegistry deviceRegistry,
//			DecodedClientMessageHandler clientMessageHandler, DeviceSessionManager sessionManager,
//			TcpServer tcpServer) {
...
//		disposable = tcpServer.handleConnection().publishOn(Schedulers.parallel())
//				.flatMap(client -> new TcpConnection(client).accept(), Integer.MAX_VALUE)
//				.onErrorContinue((err, obj) -> log.error(err.getMessage(), err))
//				.subscriberContext(ReactiveLogger.start("network", tcpServer.getId())).subscribe(ignore -> {
...
//		 */
//		Mono<Void> accept() {
//			return getProtocol().flatMap(protocol -> protocol.onClientConnect(getTransport(), client, this))
//					.then(client.subscribe().filter(tcp -> started.get()).publishOn(Schedulers.parallel())
//							.flatMap(this::handleTcpMessage).onErrorResume((err) -> {
...
//			return getProtocol().flatMap(pt -> pt.getMessageCodec(getTransport()))
//					.flatMapMany(
//							codec -> codec.decode(FromDeviceMessageContext.of(sessionRef.get(), message, registry)))
//					.cast(DeviceMessage.class).doOnNext(msg -> gatewayMonitor.receivedMessage())
//					.flatMap(this::handleDeviceMessage)
//					.doOnEach(ReactiveLogger.onError(err -> log.error("处理TCP[{}]消息失败:\n{}", address, message, err)))
//					.onErrorResume((err) -> Mono.fromRunnable(client::reset)).then();
//		}
//
//		/**
//		 * 处理设备消息
...
//					device -> new TcpDeviceSession(device, client, getTransport(), gatewayMonitor), DeviceGatewayHelper
//							.applySessionKeepaliveTimeout(message, keepaliveTimeout::get).andThen(session -> {
...
//							}),
//					() -> log.warn("无法从tcp[{}]消息中获取设备信息:{}", address, message)).then();
//		}
//
//		@Override
