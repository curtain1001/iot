...
public interface PropertyMetadataConstants {
	...
	interface Source {
		...
		static boolean isManual(DeviceMessage message) {
			return message.getHeader(Source.headerKey).map(Source.manual::equals).orElse(false);
		}
		...
		static boolean isRule(PropertyMetadata metadata) {
			return metadata.getExpand(id).map(rule::equals).orElse(false);
		}
	}
	...
	interface AccessMode {
		...
		static boolean isWrite(PropertyMetadata property) {
			return property.getExpand(id).map(val -> val.toString().contains(write)).orElseGet(() -> property
					.getExpand("readOnly").map(readOnly -> !CastUtils.castBoolean(readOnly)).orElse(true));
		}
