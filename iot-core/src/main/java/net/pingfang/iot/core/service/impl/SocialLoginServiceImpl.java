...
import static net.pingfang.iot.common.constant.HttpStatus.NO_MESSAGE_ALERT;
import static net.pingfang.iot.common.core.domain.AjaxResult.error;
...
import net.pingfang.iot.common.core.domain.model.BindLoginBody;
import net.pingfang.iot.common.core.domain.model.BindRegisterBody;
import net.pingfang.iot.common.core.domain.model.LoginUser;
...
import net.pingfang.iot.core.model.login.LoginIdValue;
import net.pingfang.iot.core.service.IAuthRequestFactory;
import net.pingfang.iot.core.service.ISocialLoginService;
import net.pingfang.iot.core.service.ISocialUserService;
import net.pingfang.iot.framework.web.service.SysLoginService;
import net.pingfang.iot.framework.web.service.SysRegisterService;
import net.pingfang.iot.framework.web.service.TokenService;
...
public class SocialLoginServiceImpl implements ISocialLoginService {
	...
	public String renderAuth(String source, HttpServletRequest httpServletRequest) {
		...
		try {
			...
			if (authRequestWrap != null) {
				...
				return httpServletRequest.getProtocol() + httpServletRequest.getServerName()
	...
	public String callback(String source, AuthCallback authCallback, HttpServletRequest httpServletRequest) {
		...
		try {
			...
			if (authResponse.ok()) {
				...
				SocialUser socialUser = findSocialUser(authResponse.getData().getUuid(),
						authResponse.getData().getSource());
	...
	public AjaxResult bindLogin(BindLoginBody bindLoginBody) {
		...
		AjaxResult ajax = AjaxResult.success();
		// 生成令牌
		String token = sysLoginService.login(bindLoginBody.getUsername(), bindLoginBody.getPassword(),
				bindLoginBody.getCode(), bindLoginBody.getUuid());
	...
	private void checkSocialPlatform(SocialPlatform socialPlatform) {
		if (socialPlatform != null && (!socialPlatform.getStatus().equals(ONLINE_STATUS)
				|| !socialPlatform.getDelFlag().equals(DEL_FLAG))) {
