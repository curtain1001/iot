...
public class EmqxStart implements ApplicationRunner {
	...
	public void run(ApplicationArguments applicationArguments) throws Exception {
		...
		/**
		 * JSONObject message = new JSONObject(); message.put("id",2);
		 * message.put("value","hello"); message.put("remark","12");
		 * Thread.sleep(10000); new Thread(() ->{ for (int i = 0; i < 3000; i++) {
		 * emqxClient.publish(0,false,"/2/D6329VL54419L1Y0/info/post",message.toJSONString());
		 * //测试多线程性能 try { Thread.sleep(300); } catch (InterruptedException e) {
