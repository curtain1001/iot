...
public class EmqxClient {
	...
	public EmqxClient(String clientId, String username, String password, String hostname, int timeout, int keepalive,
	...
	private void createClient() {
		if (client == null) {
			try {
				/*
				 * host为主机名，clientId是连接MQTT的客户端ID，MemoryPersistence设置clientId的保存方式 默认是以内存方式保存
				 */
	 ...
	 *
	 * @param topic 主题
	 * @param qos   消息质量 Qos1：消息发送一次，不确保 Qos2：至少分发一次，服务器确保接收消息进行确认
