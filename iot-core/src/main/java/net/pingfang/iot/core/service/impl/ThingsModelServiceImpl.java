...
import net.pingfang.iot.core.domain.ThingsModel;
import net.pingfang.iot.core.domain.ThingsModelTemplate;
import net.pingfang.iot.core.mapper.ProductMapper;
import net.pingfang.iot.core.mapper.ThingsModelMapper;
import net.pingfang.iot.core.mapper.ThingsModelTemplateMapper;
import net.pingfang.iot.core.model.ImportThingsModelInput;
import net.pingfang.iot.core.model.ThingsModels.EventDto;
import net.pingfang.iot.core.model.ThingsModels.FunctionDto;
import net.pingfang.iot.core.model.ThingsModels.PropertyDto;
import net.pingfang.iot.core.model.ThingsModels.ThingsModelsDto;
import net.pingfang.iot.core.service.IThingsModelService;
...
public class ThingsModelServiceImpl implements IThingsModelService {
	...
	public int importByTemplateIds(ImportThingsModelInput input) {
		...
		SysUser user = getLoginUser().getUser();
		// 根据ID集合获取通用物模型列表
		List<ThingsModelTemplate> templateList = thingsModelTemplateMapper
				.selectThingsModelTemplateByTemplateIds(input.getTemplateIds());
		// 转换为产品物模型，并批量插入
	...
	public int updateThingsModel(ThingsModel thingsModel) {
		...
		List<ThingsModel> list = thingsModelMapper.selectThingsModelList(input);
		Boolean isRepeat = list.stream().anyMatch(x -> x.getIdentifier().equals(thingsModel.getIdentifier())
				&& x.getModelId().longValue() != thingsModel.getModelId());
		if (!isRepeat) {
